...
export function createServices(env: Env): Services {
	...
	if (!herePlacesDummy) {
		...
		if (!herePlacesUrl || !herePlacesAppId || !herePlacesAppCode) {
			throw new Error('APP_HERE_PLACES_URL, APP_HERE_PLACES_APP_ID or APP_HERE_PLACES_APP_CODE is not defined.');
		}

		herePlacesClient = new HerePlacesClient(herePlacesUrl, herePlacesAppId, herePlacesAppCode);
	} else {
	...
	const model = createModel(createConnection(mongoUrl, mongoDbName, mongoUsername, mongoPassword));
