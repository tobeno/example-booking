...
export interface HerePlacesResponse {
	statusCode: number;
  body: string;
  toString(): string;
}
...
class HerePlacesClient {
	...
	async browse(at: string, categories: Array<string>, size: number): Promise<Array<HerePlacesItem>> {
	...
	async get(path: string, parameters: { [key: string]: string } = {}): Promise<HerePlacesResponse> {
		...
		try {
			...
			if (err instanceof StatusCodeError) {
				...
				throw new Error(`Request failed with error response: ${body ? body.message : 'unknown'}`);
			}
